...
Since the conductance matrix $\m{G}$ is a symmetric matrix (and the capacitance $\m{C}$ a diagonal matrix) \cite{rao2009, huang2008}, we can apply the following substitution aimed to keep the symmetry:
\begin{align}
...
\todo{Now we assume that the power profile} is evenly sampled with the sampling interval equal to $\Delta t$, i.e., $\Delta t_i = \Delta t$ for $i = 0 \dots N_s - 1$. Having this assumption, the recurrent process in \equref{eq:recurrent-equation} turns into:
\[
