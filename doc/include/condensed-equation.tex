...
Now we assume that the power profile $\mathbb{\v{P}}$ is evenly sampled with the \emph{sampling interval} equal to $\Delta t$, i.e., $\Delta t_i = \Delta t$ for $i = 0 \dots N_s - 1$. Having this assumption, the recurrent process in \equref{eq:recurrent-equation} turns into:
\[
