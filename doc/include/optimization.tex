...
where $\theta_i$ is the MTTF of the $i$th processing element given by \equref{eq:one-mttf}, $t_{\text{end} \: i}$ denotes the end time of the $i$th task, $\period$ is the period of the application, and $T_{ij}$ are temperature values in the SSDTP. \equref{eq:deadline} imposes the application deadline, which we assume to be equal to the period. \equref{eq:t-max} enforces the constraint on the maximal temperature in the temperature profile $\mathbb{T} = \{ T_{ij} \}$.
