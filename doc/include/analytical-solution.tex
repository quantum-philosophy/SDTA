...
\end{equation}
where $\mathbb{A}$ is a $N_n N_s \times N_n N_s$ matrix, $\mathbb{X}$ and $\mathbb{B}$ are vectors with $N_n N_s$ elements. It can be seen that we have obtained a regular system of linear equations with well-known. Straight-forward techniques to solve it and their disadvantages are further discussed in \appref{ap:straight-forward}.
