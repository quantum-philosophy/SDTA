In order to solve the problem, we use an analytical approach. The same technique is employed in \cite{thiele2011}. Let the power consumption vector $P(t)$ be constant and equal to $P$, then the direct solution of \equref{eq:thermal-ode} is the following:
...
All in all, we have obtained a regular system of liner equations with the SSDTP as its solution ($Y$ should also be processed with \equref{eq:finalization} in order to return back to $T$). Possible solutions include the followings:
\begin{itemize}
