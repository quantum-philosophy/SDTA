...
\image{scaling-time}{80 230 80 230}{Scalability with the application period for a single processor system. The sampling interval is fixed to 1 millisecond where 1 second on the horizontal axis corresponds to 1000 steps in the power profile. HotSpot is restricted to one iteration, the steady-state approximation is computed to each time interval. The comparison is given on the semilogarithmic scale.}
...
\image{scaling-cores}{80 230 80 230}{Scalability with the number of cores. The application period is fixed to 0.1 seconds that correspond to 100 steps in the power profile. HotSpot is restricted to one iteration, the steady-state approximation is computed to each time interval. The comparison is given on the semilogarithmic scale.}
\begin{itable}{scaling-cores}{|r|r|r|r|r|r|}
  ...
  \hline
\end{itable}
In contrast, the scalability of the methods with the number of processing elements is different, since the effect of each new core varies between the solutions. The comparison is given in \figref{fig:scaling-cores} and \tabref{tab:scaling-cores}. It can be seen that the FFT method with the increasing number of cores overtakes one HotSpot simulation, i.e., the FFT method scales better. On the other hand, the complexity of the problem for the direct solver (UMF) increases dramatically.
