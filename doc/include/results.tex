...
In this subsection we investigate the scalability properties of the proposed solution based on the condensed equation (CE) method and compare it with one transient temperature simulation (TTS) of the application period, which is not sufficient to reach the SSDTP as it was shown in \secref{sec:hotspot-solution}. For the transient temperature analysis (TTA), the HotSpot thermal simulator is used.
