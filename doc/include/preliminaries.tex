...
HotSpot comes with a tool of the same name to perform all the calculations. If we are interested in the transient temperature, it solves \equref{eq:initial} using the Runge-Kutta numerical method. When we need to get only the steady-state temperature, $\frac{dT}{dt}$ becomes zero, and \equref{eq:initial} turned into a linear system, which HotSpot solves using the LU decomposition.
