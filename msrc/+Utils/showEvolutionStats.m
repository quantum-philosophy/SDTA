function showEvolutionStats(file, remove, take)
  if nargin < 2, remove = []; end
  if nargin < 3, take = 'max'; end
  ...
  fprintf('%10s%15s%15s%15s%15s%15s%15s%15s%15s%15s\n', ...
    'No', 'Generations', 'Evaluations', 'Deadline', 'SSDTP', 'Runaways', ...
    '+LT, x', '+LTa, x', '+E, x', 'Time, s');

  SSDTP = Stats(:, E) - Stats(:, D);

  for i = 1:count
    if any(remove == i)
      fprintf('%10s%15d%15d%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', ...
        'X', Stats(i, G), Stats(i, E), Stats(i, D), SSDTP(i), ...
        Stats(i, R), Stats(i, IL), Stats(i, AL), Stats(i, IE), Stats(i, T));
    else
      fprintf('%10d%15d%15d%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', ...
        i, Stats(i, G), Stats(i, E), Stats(i, D), SSDTP(i), ...
        Stats(i, R), Stats(i, IL), Stats(i, AL), Stats(i, IE), Stats(i, T));
    end
  end
  ...
  fprintf('%10s%15d%15d%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', 'min', ...
    min(Stats(:, G)),  min(Stats(:, E)),  min(Stats(:, D)), min(SSDTP), ...
    min(Stats(:, R)),  min(Stats(:, IL)), min(Stats(:, AL)), ...
    min(Stats(:, IE)), min(Stats(:, T)));

  fprintf('%10s%15d%15d%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', 'max', ...
    max(Stats(:, G)),  max(Stats(:, E)),  max(Stats(:, D)), max(SSDTP), ...
    max(Stats(:, R)),  max(Stats(:, IL)), max(Stats(:, AL)), ...
    max(Stats(:, IE)), max(Stats(:, T)));

  fprintf('%10s%15.0f%15.0f%15.0f%15.0f%15.0f%15.2f%15.2f%15.2f%15.2f\n', 'mean', ...
    mean(Stats(:, G)),  mean(Stats(:, E)),  mean(Stats(:, D)), mean(SSDTP), ...
    mean(Stats(:, R)),  mean(Stats(:, IL)), mean(Stats(:, AL)), ...
    mean(Stats(:, IE)), mean(Stats(:, T)));

  fprintf('%10s%15s%15s%15.2f%15.2f%15s%15s%15s%15s%15s\n', 'percent', '',  '', ...
    mean(Stats(:, D)) / mean(Stats(:, E)) * 100, ...
    mean(SSDTP) / mean(Stats(:, E)) * 100, ...
    '', '', '', '', '');
