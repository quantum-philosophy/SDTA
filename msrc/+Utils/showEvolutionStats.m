function showEvolutionStats(file, full)
  ...
  fprintf('%10s%15s%15s%15s%15s%15s%15s%15s\n', ...
    ...
    'Lifetime', '+LT, %', '+E, %', 'Time, m');
  for i = 1:count
    fprintf('%10d%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', ...
      i, generations(i), evaluations(i), deadline_misses(i), ...
      lifetime(i), improvement(i, 1), improvement(i, 2), time(i));
  end

  fprintf('\n');

  fprintf('%10s%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', 'min', ...
    min(generations), min(evaluations), min(deadline_misses), ...
    min(lifetime), min(improvement(:, 1)), min(improvement(:, 2)), min(time));

  fprintf('%10s%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', 'max', ...
    max(generations), max(evaluations), max(deadline_misses), ...
    max(lifetime), max(improvement(:, 1)), max(improvement(:, 2)), max(time));

  fprintf('%10s%15d%15d%15d%15.2f%15.2f%15.2f%15.2f\n', 'mean', ...
    round(mean(generations)), round(mean(evaluations)), ...
    round(mean(deadline_misses)), mean(lifetime), ...
    mean(improvement(:, 1)), mean(improvement(:, 2)), mean(time));
