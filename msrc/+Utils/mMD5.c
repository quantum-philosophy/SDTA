...
static void MDSilentString(const char *input, char *output, int size)
{
	MD_CTX context;
	unsigned int i;
	unsigned char digest[16];
	...
	MDInit(&context);
	MDUpdate(&context, (unsigned char *)input, size);
	MDFinal(digest, &context);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (nrhs != 1) mexErrMsgTxt("There should be only one input.");

	input = mxGetPr(prhs[0]);
	output = mxCalloc(32 + 1, sizeof(char));
	output[32] = '\0';
	size = mxGetM(prhs[0]) * mxGetN(prhs[0]) * sizeof(double);

    MDSilentString((char *)input, output, size);
